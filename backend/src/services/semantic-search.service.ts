...
export async function performSemanticSearch(
  ...
  return tracer.startActiveSpan('search.semantic', async (span) => {
    if (!query || !query.trim()) {
      span.setAttributes({ 'search.query': query, 'search.result_count': 0 })
      span.end()
    ...
    span.setAttributes({ 'search.query': query, 'search.intent_tags': expandedTags.join(',') })
    ...
    try {
      ...
      if (expandedTags.length > 0) {
        ...
        span.setAttribute('search.tag_search_count', rawTracks.length)
      }
      ...
      if (rawTracks.length < 5) {
        ...
        span.setAttribute('search.standard_search_count', standardTracks.length)
      }
      ...
      span.setAttributes({
        'search.result_count': paginated.length,
        'search.is_fallback': false,
        'search.avg_semantic_score': paginated.length > 0 ? Math.round(paginated.reduce((sum, r) => sum + r.semanticScore, 0) / paginated.length * 100) / 100 : 0,
      })
      span.end()
      return {
        query,
      ...
      span.setAttributes({ 'search.result_count': fallbackTracks.length, 'search.is_fallback': true })
      span.end()
      return {
        query,
        intentTags: [],
