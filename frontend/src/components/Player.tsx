...
export default function Player() {
  ...
  // 3. Phím tắt Bàn phím (Space = Play/Pause, ArrowLeft/Right = Tua 5s)
  useEffect(() => {
    ...
    <>
      ...
      <div className="px-4 pb-4 w-full z-50">
        <div className="h-24 md:h-20 bg-white/10 backdrop-blur-2xl border border-white/20 flex flex-col md:flex-row items-center justify-between px-6 py-2 md:py-0 w-full rounded-3xl shadow-[0_10px_40px_rgba(0,0,0,0.5)] relative overflow-hidden">
          ...
          {/* BÊN TRÁI */}
          <div className="flex items-center gap-4 w-full md:w-1/3 mb-2 md:mb-0 relative z-10">
            ...
            <button
              ...
              className="text-white/40 hover:text-pink-500 transition-colors flex-shrink-0 p-1"
          ...
          <div className="flex flex-col items-center max-w-[400px] w-full relative z-10">
            <div className="flex items-center gap-8 mb-1">
              <button 
                ...
                className={`transition-all p-1 ${isShuffle ? 'text-indigo-400 drop-shadow-[0_0_8px_rgba(99,102,241,0.8)]' : 'text-white/40 hover:text-white'}`}
                title={isShuffle ? "Tắt trộn bài" : "Bật trộn bài"}
              >
                <Shuffle className="w-4 h-4" />
              </button>
              
              <button onClick={prevTrack} className="text-white/60 hover:text-white transition-all">
                <SkipBack className="w-5 h-5 fill-current" />
              </button>
              
              <button onClick={togglePlay} className="w-10 h-10 flex items-center justify-center bg-white rounded-full hover:scale-105 transition-transform shadow-[0_0_20px_rgba(255,255,255,0.3)]">
                {isPlaying ? <Pause className="w-5 h-5 fill-black text-black" /> : <Play className="w-5 h-5 fill-black text-black ml-0.5" />}
              </button>
              
              <button onClick={nextTrack} className="text-white/60 hover:text-white transition-all">
                <SkipForward className="w-5 h-5 fill-current" />
              </button>

              <button 
                onClick={toggleRepeat} 
                className={`transition-all p-1 ${repeatMode !== 'off' ? 'text-indigo-400 drop-shadow-[0_0_8px_rgba(99,102,241,0.8)]' : 'text-white/40 hover:text-white'}`}
          ...
          {/* BÊN PHẢI */}
          <div className="hidden md:flex items-center justify-end gap-4 w-1/3 text-white/50 relative z-10">
            <button 
