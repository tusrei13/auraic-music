...
const configs: Record<SurfaceKind, { eyebrow: string; title: string; description: string; icon: typeof Disc3; tags?: string; accent: string }> = {
  genres: { eyebrow: "Jamendo catalog", title: "Explore by genre", description: "Browse a living catalog of independent music, grouped by the feeling you came for.", icon: Disc3, accent: "fuchsia" },
  ...
  search: { eyebrow: "Deep search", title: "Find your frequency", description: "Search the Jamendo catalog with room for context, not just keywords.", icon: Search, accent: "violet" },
};
...
export default function CatalogSurface({ kind }: { kind: SurfaceKind }) {
  ...
  return (
    <div className="min-h-full overflow-y-auto px-5 pb-36 pt-8 text-white sm:px-8 lg:px-12">
      ...
      {kind === "radio" ? <div className="mt-8 grid gap-4 sm:grid-cols-2">{stations.map((station, index) => <button key={station.name} type="button" onClick={() => void playStation(station.tags, station.name)} className={`group relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br ${index % 2 === 0 ? "from-cyan-400/15 to-white/[0.03]" : "from-fuchsia-400/15 to-white/[0.03]"} p-5 text-left transition hover:-translate-y-1 hover:border-cyan-300/50`}><span className="absolute -right-5 -top-8 h-32 w-32 rounded-full border border-white/10 bg-white/[0.05]" /><span className="relative flex h-12 w-12 items-center justify-center rounded-2xl bg-cyan-300/10 text-cyan-200"><Radio className="h-5 w-5" /></span><h2 className="relative mt-8 text-lg font-bold">{station.name}</h2><p className="relative mt-1 text-sm text-white/45">{station.note}</p><span className="relative mt-5 inline-flex items-center gap-2 text-xs font-bold text-cyan-200"><Play className="h-3.5 w-3.5 fill-current" /> Start station <span className="text-white/25">·</span> 24/7</span></button>)}</div> : null}
