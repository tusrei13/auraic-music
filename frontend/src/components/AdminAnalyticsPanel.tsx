...
export default function AdminAnalyticsPanel() {
  ...
  const quality = analytics?.quality || { invalidTitle: 0, invalidTiming: 0, unknownSource: 0, duplicateStarted: 0, totalIssues: 0 };
  ...
  return <section className="mt-8 grid gap-5 lg:grid-cols-[1.2fr_0.8fr]">
    ...
    <div className="space-y-5"><article className="rounded-2xl border border-white/10 bg-black/20 p-6"><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-fuchsia-300">Top tracks</p><h2 className="mt-1 text-xl font-black">Most started</h2>{analytics?.topTracks.length ? <div className="mt-5 space-y-4">{analytics.topTracks.slice(0, 5).map((track, index) => <div key={track.trackId} className="flex items-center gap-3"><span className="w-5 text-xs text-white/30">{String(index + 1).padStart(2, "0")}</span><span className="min-w-0 flex-1 truncate text-sm">{track.title}</span><span className="text-xs text-cyan-200">{track.plays}</span></div>)}</div> : <p className="mt-8 text-sm leading-6 text-white/40">Chưa có playback event trong 7 ngày gần nhất.</p>}</article><article className={`rounded-2xl border p-6 ${quality.totalIssues ? "border-amber-300/25 bg-amber-300/[0.06]" : "border-emerald-300/20 bg-emerald-300/[0.05]"}`}><div className="flex items-center justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/45">Data quality</p><h2 className="mt-1 text-xl font-black">{quality.totalIssues ? `${quality.totalIssues} issues found` : "Healthy events"}</h2></div><span className="text-2xl font-black">{quality.totalIssues}</span></div><div className="mt-5 grid grid-cols-2 gap-3 text-xs text-white/55"><span>Invalid title <b className="ml-1 text-white">{quality.invalidTitle}</b></span><span>Invalid timing <b className="ml-1 text-white">{quality.invalidTiming}</b></span><span>Unknown source <b className="ml-1 text-white">{quality.unknownSource}</b></span><span>Duplicate starts <b className="ml-1 text-white">{quality.duplicateStarted}</b></span></div></article></div>
  </section>;
}
